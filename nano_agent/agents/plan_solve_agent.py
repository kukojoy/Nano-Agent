...
class Planner:
    ...
    def __init__(self, llm: NanoAgentLLM, prompt_template: Optional[str] = None):
        self.llm_client = llm
    ...
    def plan(self, question: str, **kwargs) -> list[str]:
        ...
        messagea = [{"role": "system", "content": prompt}]

        print("--- 正在生成计划 ---")
        response_text = self.llm_client.think(messagea, **kwargs) or ""
        print(f"✅ 计划已生成: \n{response_text}")
...
class Executor:
    ...
    def execute(self, question: str, plan: List[str], **kargs) -> str:
        """执行计划
        ...
        for i, step in enumerate(plan):
            ...
            response_text = self.llm_client.think(messages, **kargs) or ""
