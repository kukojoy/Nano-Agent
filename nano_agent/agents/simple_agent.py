...
class SimpleAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            system_prompt: 系统提示词
            config: 配置对象
            tool_registry: 可选的工具注册表, 用于管理可用的工具
            enable_tool_calling: 是否启用工具调用功能, 只有在提供tool_registry时生效, 默认为True
        """  
    ...
    def add_tool(self, tool, auto_expand: bool = True) -> None:
        ...
        Args:
            tool: 工具实例
            auto_expand: 是否自动将可展开的工具展开为子工具集, 默认为True
        """
        ...
        # 直接使用 ToolRegistry 注册工具, 会自动处理工具展开
        self.tool_registry.register_tool(tool, auto_expand)
    ...
    def _parse_tool_parameters(self, tool_name: str, parameters: str) -> dict:
        ...
        # 尝试解析格式: key=value 或 action=search,query=Python
        if '=' in parameters:
            if ',' in parameters:  # 多个参数: action=search,query=Python,limit=3
