...
class EpisodicMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 统一嵌入模型(多语言, 默认384维) TODO: check这里的默认到底是怎么默认的
        self.embedder = get_text_embedder()

        # 向量存储(Qdrant) TODO: 理解为什么有了SQLite还要Qdrant, 以及它们的关系
        ...
        self.vector_store = QdrantConnectionManager.get_instance(
            ...
            collection_name=os.getenv("QDRANT_COLLECTION", "nano_agent_vectors"),
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        # 1) memory_item -> Episode -> 内存存储
        ## 从 memory_item 的 metadata 中提取情景信息
        ...
        if session_id not in self.sessions:  # 如果 session_id 不存在, 则创建一个新的 session
            self.sessions[session_id] = []
        self.sessions[session_id].append(episode.episode_id)

        # 2) 权威文档存储(SQLite) TODO: 查询"权威"的意思
        ...
        # 3) 向量存储(Qdrant): 将 memory_item.content 转换为向量, 并存储到 Qdrant 中, 以支持后续的语义检索
        try:
        ...
        if not results:
            ...
            for ep in self._filter_episodes(user_id, session_id, time_range):
                if query_lower in ep.content.lower():
                    # 回退匹配: 新评分算法
                    keyword_score = 0.5  # 简单关键词匹配的基础分数 BUG: keyword_score 为固定值
    ...
    def update(
        ...
        updated = False
        for episode in self.episodes:  # 更新内存存储
        ...
        return updated or doc_updated  # NOTE: 无法体现内存或数据库的具体更新状态
    ...
    def remove(self, memory_id: str) -> bool:
        ...
        return removed or doc_deleted  # NOTE: 无法体现内存或数据库的具体删除状态
    
    def has_memory(self, memory_id: str) -> bool:
        """检查记忆是否存在"""
        return any(episode.episode_id == memory_id for episode in self.episodes)  # NOTE: 只检查了内存
    ...
    def clear(self):
        ...
        for m_id in ids:
            self.doc_store.delete_memory(m_id)
        ...
        for episode in self.episodes:
            ...
            if strategy == "importance_based":  # 基于重要性遗忘
                ...
                if episode.timestamp < cutoff_time:
                    should_forget = True
            elif strategy == "capacity_based":  # 基于容量遗忘(保留最重要的)
                # TODO: check实现效率
    ...
    def get_all(self) -> List[MemoryItem]:
        ...
        for episode in self.episodes:
            memory_item = MemoryItem(
                ...
                metadata=episode.metadata  # TODO: check Episode 的 metadata 属性定义了吗?
            )
            memory_items.append(memory_item)
        return memory_items
        
    def get_stats(self) -> Dict[str, Any]:
        """获取情景记忆的统计信息(合并SQLite与Qrant)"""
        active_episodes = self.episodes  # 在硬删除策略下, self.episodes 已经是当前有效的记忆列表
    ...
    def find_patterns(self, user_id: str = None, min_frequency: int = 2) -> List[Dict[str, Any]]:
        """发现用户行为模式, 基于情景记忆内容和上下文识别频繁出现的关键词和情景模式"""
        # 检查缓存
    ...
    def _filter_episodes(
        ...
        """过滤情景, 基于 user_id, session_id 和 time_range"""
