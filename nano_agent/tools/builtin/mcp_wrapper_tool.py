"""MCP 工具包装器 - 将单个 MCP 工具包装成 NanoAgent Tool

这个模块将 MCP 服务器的每个工具展开为独立的 NanoAgent Tool对象,
...
class MCPWrappedTool(Tool):
    """MCP工具包装器 - 将单个MCP工具包装成 NanoAgent Tool
    ...
    def _parse_input_schema(self, input_schema: Dict[str, Any]) -> List[ToolParameter]:
        """将 MCP 的 input_schema 转换为 NanoAgent 的 ToolParameter 列表
