...
提供强化学习训练功能, 包括 SFT/GRPO/PPO 等算法.
"""
...
class RLTrainingTool(Tool):
    ...
    def __init__(self):
        ...
        # 检查 TRL 是否可用
        try:
            from nano_agent.rl import TRL_AVAILABLE
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        Args: TODO: 这里的 Args 列表写得很好, 后续可以参考这个格式完善其他工具
            parameters: 操作参数, 包括:
    ...
    def _handle_load_dataset(self, parameters: Dict[str, Any]) -> str:
        """处理数据集加载操作"""
        from nano_agent.rl import create_sft_dataset, create_rl_dataset
    ...
    def _handle_create_reward(self, parameters: Dict[str, Any]) -> str:
        ...
        创建了 reward_fn 但没有返回 ? 那这个函数的意义是什么 ?
        """
        from nano_agent.rl import (
    ...
    def get_parameters(self) -> List[ToolParameter]:
        ...
        return [
            ...
            ToolParameter(
                name="max_samples",
                type="integer",
                description="最大样本数 (用于快速测试), None 表示使用全部数据",
                required=False,
                default=None
            ),
            ToolParameter(
