...
class RAGTool(Tool):
    ...
    def _init_components(self):
        """初始化 RAG 组件: pipeline + llm"""

        try:
            # 初始化默认命名空间的 RAG pipeline
            ...
            self._pipelines[self.rag_namespace] = default_pipeline  # self._pipelines: namespace -> pipeline

            # 初始化 LLM 用于回答生成
            self.llm = NanoAgentLLM()
    ...
    def _clear_knowledge_base(self, confirm: bool = False, namespace: str = "default") -> str:
        ...
        try:
            ...
            pipeline = self._get_pipeline(namespace)
            store = pipeline.get("store")  # Qdrant
