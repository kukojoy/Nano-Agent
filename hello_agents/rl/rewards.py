...
class MathRewardFunction:
    ...
    def __init__(self, tolerance: float = 1e-4):
        ...
        Args:
            tolerance: 对于数学问题, 数值结果的比较容差
        """

        self.tolerance = tolerance
        self.__name__ = "MathRewardFunction" # ? 这个不是和类名一样吗 ?
    ...
    def __call__(
        ...
        **kwargs
        ...
        Args:
            ...
            kwargs: 其他参数, 需要包含 "ground_truths" 键
            
        Returns:
            奖励列表, 每个元素对应一个 completion 的奖励, 1.0表示正确, 0.0表示错误
        """
        # 从 kwargs 中获取 ground_truths
        ground_truths = kwargs.get("ground_truths", [])
        if not ground_truths:
            raise ValueError("ground_truths 必须在数据集中提供")
