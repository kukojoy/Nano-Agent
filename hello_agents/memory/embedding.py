...
说明:
- 通过统一的文本嵌入接口与多实现: 本地Transformer, DashScope(通义千问), TF-IDF兜底
- 暴露 get_text_embedder() / get_dimension() / refresh_embedder() 供各记忆类型统一使用
- 环境变量优先级: dashscope > local > tfidf

环境变量:
- EMBED_MODEL_TYPE: "dashscope" | "local" | "tfidf" (默认dashscope)
- EMBED_MODEL_NAME: 模型名称(dashscope 默认 text-embbedding-v3, local 默认 sentence-transformers)
...
class TFIDFEmbedding(EmbeddingModel):
    ...
    def _init_vectorizer(self):
        try:
            ...
            self._vectorizer = TfidfVectorizer(max_features=self.max_features, stop_words='english')  # stop_words表示去除英文停用词, 可根据需要调整或移除
        except ImportError:
            raise ImportError("请安装 scikit-learn 以使用 TF-IDF 嵌入: pip install scikit-learn")
    ...
    def encode(self, texts: Union[str, List[str]]):
        if not self._is_fitted:
            raise ValueError("TF-IDF 模型未拟合, 请先调用 fit() 方法")
...
class LocalTransformerEmbedding(EmbeddingModel):
    ...
    优先使用 sentence-transformers, 缺失时回退 transformers + torch    
    """

    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        self.model_name = model_name
        self._backend = None  # "st"(sentence_transformers) or "hf"(huggingface)
        self._st_model = None
        self._hf_model = None
    ...
    def _load_backend(self):
        # 优先尝试加载 sentence-transformers 库
        try:  
            from sentence_transformers import SentenceTransformer
            self._st_model = SentenceTransformer(self.model_name)
            test_vec = self._st_model.encode("dummy_input")
        ...
        print("使用 sentence-transformers 失败, 尝试使用 transformers + torch...")
        # 尝试使用 transformers
        try:
            from transformers import AutoModel, AutoTokenizer
            import torch
            self._hf_tokenizer = AutoTokenizer.from_pretrained(self.model_name)  # 分词器
            self._hf_model = AutoModel.from_pretrained(self.model_name)          # 嵌入模型
           
            with torch.no_grad():
                inputs = self._hf_tokenizer("dummy_input", return_tensors="pt", padding=True, truncation=True)
    ...
    @property
    def dimension(self) -> int:
        return int(self._dimension or 0)  # TODO check为什么or 0
    ...
    def encode(self, texts: Union[str, List[str]]):
        ...
        if self.base_url:
            ...
            if response.status_code >= 400:
                raise RuntimeError(f"Embedding REST 调用失败: {response.status_code} {response.text}")
            data = response.json()
...
def create_embedding_model(model_type: str = "local", **kwargs) -> EmbeddingModel:
    ...
    Args:
        ...
        **kwargs: 传递给具体模型的参数, 如 model_name/api_key/base_url 等
    """
    if model_type == "dashscope":
        return DashScopeEmbedding(**kwargs)
    elif model_type in ("local", "sentence_transformer", "huggingface"):
