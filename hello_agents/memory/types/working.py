...
class WorkingMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        ## 纯内存TTL(分钟), 可通过在 MemoryConfig 中配置 working_memory_ttl_minutes 来覆盖
        self.max_age_minutes = getattr(self.config, 'working_memory_ttl_minutes', 120)
        
        self.current_tokens = 0
        self.session_start = datetime.now()  # 会话开始时间

        # 内存存储(工作记忆不需要持久化)
        self.memories: List[MemoryItem] = [] 

        # 使用优先队列管理记忆
        self.memory_heap = []  # 元素格式: (-priority, timestamp, memory_item), 负优先级实现最大堆
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        # 计算优先级(重要性 + 时间衰减), 并添加到优先队列和记忆内存列表
    ...
    def retrieve(self, query: str, limit: int = 5, user_id: str = None, **kwargs) -> List[MemoryItem]:
        ...
        try:
            ...
            tfidx_matrix = vectorizer.fit_transform(documents)

            # 计算余弦相似度
            query_vector = tfidx_matrix[0:1]
            doc_vectors = tfidx_matrix[1:]
        ...
        for memory in filtered_memories:
            ...
            ### 计算关键词匹配分数
            keyword_score = 0.0
            if query_lower in content_lower:  # 全词匹配
                keyword_score = len(query_lower) / len(content_lower)
            else:  # 分词匹配
                query_words = set(query_lower.split())
                content_words = set(content_lower.split())
                intersection = query_words.intersection(content_words)  # 求交集
                if intersection:
                    keyword_score = len(intersection) / len(query_words.union(content_words)) * 0.8  # 求交并比, 权重为0.8
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        active_memories = self.memories  # NOTE 需要检查retrieve方法中 [过滤遗忘记忆逻辑] 的实现方式

        return {
            "count": len(active_memories),          # 活跃记忆数量
            "forgotten_count": 0,                   # 工作记忆中, 已遗忘的记忆会被直接删除
    ...
    def _mark_deleted_in_heap(self, memory_id: str):
        """在堆中标记删除的记忆"""
        # 由于heapq不支持直接删除, 我们标记为已删除, 在后续操作中清理
        # TODO 未实现
        pass
