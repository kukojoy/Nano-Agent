...
结合 [向量检索] 和 [知识图谱] 的混合语义记忆, 使用:
- HuggingFace 中文预训练模型进行文本嵌入
- 向量相似度检索进行快速初筛
- Neo4j图数据库构建知识图谱, 进行实体关系推理
- 混合检索策略优化结果质量
"""
...
# 配置日志, TODO: 留意这里为什么变得比其他记忆更复杂了, 可能是因为涉及到向量检索和图数据库的交互, 需要更多的调试信息来跟踪流程和性能瓶颈
...
class SemanticMemory(BaseMemory):
    ...
    TODO: 对比 perceptual 的 embedding, 但其实最开始 import 了 ..embedding, 其中只有本地方式是 HuggingFace 模型
    ...
    def _init_embedding_model(self):
        """初始化嵌入模型(由 embedding_provider 管理)"""
        try:
    ...
    def _init_databases(self):
        """初始化专业存储数据库
        TODO: 对比思考为啥前面的记忆类型没实现专门的数据库初始化
        """
        try:
            from ...core.database_config import get_database_config
            db_config = get_database_config()  # 获取数据库配置
    ...
    def update(
        ...
        try:
            if content is not None:
                ...
                # 更新Neo4j图数据库 TODO: 更新Neo4j在哪? 为什么不用_add_entity_to_graph?
                for entity in entities:
    ...
    def forget(self, strategy: str = "importance_based", threshold: float = 0.1, max_age_days: int = 30) -> int:
        ...
        for memory in self.semantic_memories:
            ...
            if strategy == "importance_based":  # 基于重要性遗忘
                if memory.importance < threshold:
                    should_forget = True
            elif strategy == "time_based":  # 基于时间遗忘
                cutoff_time = current_time - timedelta(days=max_age_days)
                if memory.timestamp < cutoff_time:
                    should_forget = True
            elif strategy == "capacity_based":  # 基于容量遗忘
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        active_memories = self.semantic_memories  # 硬删除模式: 所有记忆都是活跃的
    ...
    def _graph_search(self, query: str, limit: int, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 构建结果 - 从向量数据库获取完整记忆 TODO: 哪来的向量数据库?
            results = []
            for memory_id in list(related_memory_ids)[:limit * 2]:  # 获取更多候选
                try:
                    # 优先从本地缓存获取记忆详情, 避免占位向量维度不一致的问题
    ...
    def _calculate_graph_relevance_neo4j(self, memory_metadata: Dict[str, Any], query_entities: List[Entity]) -> float:
        ...
        try:
            ...
            entity_score = matching_entities / len(query_entity_ids) if query_entity_ids else 0  # 计算匹配实体占查询实体的比例

            # 实体数量加权
            entity_count = memory_metadata.get("entity_count", 0)
            entity_density = min(entity_count / 10, 1.0)  # 归一化到[0, 1] TODO: 分母10是怎么来的

            # 关系数量加权
            relation_count = memory_metadata.get("relation_count", 0)
            relation_density = min(relation_count / 5, 1.0)  # 归一化到[0, 1] TODO: 分母5是怎么来的
    ...
    def _combine_and_rank_results(
        ...
        for result in vector_results:
            ...
            # 内容去重: 检查是否已经有相同或高度相似的内容 TODO: hash可以实现高度相似内容去重吗? 
    ...
    def _add_or_update_entity(self, entity: Entity):
        """添加或更新实体"""
        if entity.entity_id in self.entities:  # 更新现有实体
    ...
    def _store_linguistic_analysis(self, doc, text: str):
        ...
        try:
            # 为每个词元创建节点
            for token in doc:
                if token.is_punct or token.is_space:  # 跳过标点和空格
                    continue
