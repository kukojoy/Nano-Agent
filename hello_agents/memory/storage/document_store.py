...
class SQLiteDocumentStore(DocumentStore):
    ...
    def __init__(self, db_path: str = "./memory.db"):
        ...
        self.local = threading.local()  # 每个线程独立的数据库连接
    ...
    def _get_connection(self):
        ...
        if not hasattr(self.local, "connection"):
            ...
            self.local.connection.row_factory = sqlite3.Row  # 以字典形式返回结果, 使结果可以按列名访问
    ...
    def add_memory(
        ...
        """, (
            memory_id, user_id, content, memory_type, timestamp, importance, json.dumps(properties) if properties else None
        ))
