...
class GAIADataset:
    ...
    GAIA 是一个通用 AI 助手评估基准, 包含 466 个真实世界问题,
    ...
    def _load_from_huggingface(self) -> List[Dict[str, Any]]:
        ...
        注意: GAIA 是 gated dataset, 需要 HF_TOKEN 环境变量
        使用 snapshot_download 下载整个数据集到本地
        """
        try:
            ...
            if not hf_token:
                ...
                print("GAIA 是 gated dataset, 需要在 HuggingFace 上申请访问权限")
                print("然后设置环境变量: HF_TOKEN=your_token")
                return []
            
            # 下载数据集到本地
            print(f"📥 下载 GAIA 数据集...")
            # 使用当前工作目录下的 data/gaia 文件夹
            ...
            try:
                ...
                print(f"⚠️ 下载失败: {e}")
                print("请确保:")
                print("1. 已在 HuggingFace 上申请 GAIA 访问权限")
