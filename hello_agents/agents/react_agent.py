...
class ReActAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            config: 配置对象
            max_steps: 最大执行步骤数
            custom_prompt: 自定义ReAct提示词模板
        """

        super().__init__(name, llm, system_prompt, config)

        # 如果未提供工具注册表, 创建一个空的注册表
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        while current_step < self.max_steps:
            ...
            # 构建提示词
            tools_desc = self.tool_registry.get_tools_description()  # 获取工具描述
            history_str = "\n".join(self.current_history)  # 构建历史记录
