...
为框架提供记忆能力, 可作为工具添加到任何Agent中, 让Agent具备记忆功能
"""
...
class MemoryTool(Tool):
    ...
    def _add_memory(
        ...
        Args:
            content: 记忆内容
            memory_type: 记忆类型: working, episodic, perceptual, semantic
            importance: 重要性分数, 0.0-1.0
            file_path: 感知记忆: 本地文件路径 (image/audio)
            modality: 感知记忆模态: text/image/audio
        ...
        try:
            ...
            if memory_type == "perceptual" and file_path:
                inferred = modality or self._infer_modality(file_path)  # 自动推断模态
    ...
    def auto_record_conversation(self, user_input: str, agent_response: str):
        ...
        # 记录用户输入 TODO: type 和 conversation_id 接口是否正确?
        self._add_memory(
    ...
    def add_knowledge(self, content: str, importance: float = 0.9):
        ...
        便捷方法, 用于添加重要知识
        """
        return self._add_memory(  # TODO: knowledge_type 参数 和 source 参数 哪来的, 有啥用
            content=content,
    ...
    def get_context_for_query(self, query: str, limit: int = 3) -> str:
        ...
        这个方法可以被Agent调用来获取相关的记忆上下文, TODO: 为啥不用 _search_memory 方法
        """
        results = self.memory_manager.retrieve_memories(
            query=query,
            limit=limit,
    ...
    def consolidate_memories(self): 
        """整合记忆 TODO: 为啥不用 _consolidate 方法"""
